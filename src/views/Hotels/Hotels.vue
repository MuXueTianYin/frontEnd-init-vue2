<template>
  <page-header-wrapper>
    ...
    <div class="table-operator">
      <a-button type="primary" icon="plus" @click="openHotelForm()">新建</a-button>
    </div>
    <a-table :columns="columns" :data-source="data" :pagination="pagination" @change="handleTableChange" :loading="loading">
      <a slot="name" slot-scope="text">{{ text }}</a>
      <span slot="action" slot-scope="text, record">
        <a @click="openHotelForm(record)">edit</a>
        <a-divider type="vertical" />
        <a @click="deletable(record)">Delete</a>
      </span>
    </a-table>
    <HotelForm :visible.sync="showHotelForm" :record="currentRecord" @update:visible="closeHotelForm" @update-success="fetchData"></HotelForm>
  </page-header-wrapper>
</template>

<script>
import { list, Delete } from '@/api/hotels'
import HotelForm from './modules/HotelsForm'
